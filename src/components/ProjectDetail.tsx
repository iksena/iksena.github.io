...
export const ProjectDetail = ({ project, onClose }: ProjectDetailProps): ReactElement => (
  <motion.div
  ...
  >
    <button
      type="button"
      onClick={onClose}
      className="absolute top-4 right-4 p-2 bg-white/80 hover:bg-white rounded-full z-20 shadow-sm"
    >
    ...
    <div className="flex gap-2 mb-6 flex-wrap">
      {project.stack.map((tech) => (
        <span key={tech} className="px-3 py-1 bg-[#E8DCCA] text-xs rounded-full font-medium text-[#4B3832]">
          {tech}
        </span>
      ))}
    </div>
    ...
    <div className="flex gap-4">
      {project.learnMoreLink && (
        <button
          ...
          onClick={() => openLink(project.learnMoreLink!)}
        >
      ...
      {project.demoLink && (
        <button
          type="button"
          className={`flex items-center gap-2 px-6 py-3 border-2 ${THEME.border} ${THEME.text} rounded-xl font-semibold hover:bg-[#E8DCCA]`}
